...
| 8 | Betrieb | Sicherung der Datenbank, Überwachung des Feeds, Datenschutzerklärung, Secrets | M | offen — Feed-Alarm (`AlertSender`/`AlertMailSender`, drei fehlgeschlagene Läufe in Folge, `docs/betrieb-tippspiel.md`) umgesetzt und getestet (2026-08-18), Sicherung weiterhin nur als Runbook vorbereitet; Datenschutzerklärung (`docs/datenschutzerklaerung.md`) und Impressum (`docs/impressum.md`, Angaben von fourteen-it.de) als Entwürfe vorbereitet, ins Frontend eingebunden (2026-08-18) — Auftragsverarbeitungsverträge, Drittlandtransfer bei Fly.io und rechtliche Prüfung bleiben offen; Secrets (Postgres, SMTP, `admin.email`, `alert.email`) weiterhin offen |
