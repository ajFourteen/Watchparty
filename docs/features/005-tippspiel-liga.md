...
| 8 | Betrieb | Sicherung der Datenbank, Überwachung des Feeds, Datenschutzerklärung, Secrets | M | offen — Feed-Alarm (`AlertSender`/`AlertMailSender`, drei fehlgeschlagene Läufe in Folge, `docs/betrieb-tippspiel.md`) umgesetzt und getestet (2026-08-18); Datenbank-Anbieter entschieden (unmanaged Fly Postgres statt Managed Postgres, Strato-Hosting-DB verworfen — MySQL statt Postgres, private IP nicht erreichbar; ADR-035-Nachtrag, 2026-08-18), Einrichtung selbst noch offen; Datenschutzerklärung (`docs/datenschutzerklaerung.md`) und Impressum (`docs/impressum.md`, Angaben von fourteen-it.de) als Entwürfe vorbereitet, ins Frontend eingebunden (2026-08-18) — Auftragsverarbeitungsverträge, Drittlandtransfer bei Fly.io und rechtliche Prüfung bleiben offen; Secrets (Postgres, SMTP, `admin.email`, `alert.email`) weiterhin offen |
