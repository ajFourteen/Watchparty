...
**Nur das Admin-Konto darf einen Handeintrag setzen.**
Angenommen Anna ist ein angemeldeter Tipper, aber nicht das konfigurierte
Admin-Konto.
Wenn sie versucht, ein Endergebnis von Hand zu setzen.
Dann wird das abgelehnt, und das Ergebnis des Spiels bleibt unverändert.
...
adapter/out/feed/    ESPN-Client und Mapping. Kein interner Nachführ-Job
                     mehr (ADR-037-Nachtrag, 2026-08-18): ESPN blockiert
                     Fly.ios IP-Bereich, ein täglicher GitHub-Actions-Workflow
                     ruft stattdessen von dort ab
...
| 3 | Konten | Magic Link, Mailversand, Sitzung, Rate Limit, Löschung | M | **erledigt** (2026-08-17, Mailversand ergänzt 2026-08-18): `LoginLink`/`LoginLinkToken`, `AccountSession`/`SessionToken`, `ClientIp`, `LoginService` (`LoginCommands`), Rate Limit im Arbeitsspeicher, Kapitel 13.2 in `anforderungen.md`, Anhang A 13.2-a bis 13.2-h. Mailversand über `SmtpMailSender` (IONOS, richtiggestellt am 2026-08-18 — die Rückfrage vom 2026-08-17 hatte fälschlich Strato genannt) mit `LoggingMailSender` als Rückfallebene ohne Zugangsdaten |
| 4 | Spieldaten | Feed-Anbindung, Nachführ-Job, Handeintrag, Umgang mit Verlegung, Absage, Korrektur | M | **erledigt** (2026-08-17): `Game`/`GameId`/`GameStatus`, `Matchday`, `Team`/`TeamId`, `SeasonId`, `EspnScheduleFeed` gegen aufgezeichnete Antworten, Kapitel 13.3 in `anforderungen.md`, Anhang A 13.3-a bis 13.3-h. Der ursprüngliche interne `ScheduleSyncJob` (Nachführ-Job über den geteilten `Scheduler`-Port) ist seit 2026-08-18 entfernt — ESPN blockiert Fly.ios IP-Bereich mit 403; ersetzt durch einen täglichen GitHub-Actions-Relay (`.github/workflows/schedule-relay.yml`, ADR-037-Nachtrag) über `ScheduleFeed.parseExternalResponse`/`ScheduleCommands.ingestRelayedFeed` |
...
| 7 | Oberfläche | Moduswechsel, Anmeldung, Spieltagsansicht, Ranglisten | L | **erledigt** (2026-08-18): `adapter/in/http` (Login-/Prediction-/LeagueController, Sitzungscookie über `AccountArgumentResolver`), `frontend/src/league` (Anmeldung, Spieltag, Ligen, Ranglisten), Moduswechsel in `App.jsx`. `LeagueHttpFlowTest` prüft den REST-Weg end-to-end gegen echtes Postgres; das Frontend ist nur bis `npm run build` und manuell per `curl` durch den vollständigen Ablauf verifiziert, nicht in einem echten Browser — dafür fehlt in dieser Umgebung ein Werkzeug (Playwright o. ä.). Handeintrag-Endpunkt (Kriterium 14/13.3-g/13.3-h) ergänzt am selben Tag: `ScheduleController`, geschützt über ein fest konfiguriertes Admin-Konto (`watchparty.league.admin.email`, ADR-036), geprüft in `ScheduleControllerHttpTest` (Admin darf, gewöhnlicher Tipper nicht) |
| 8 | Betrieb | Sicherung der Datenbank, Überwachung des Feeds, Datenschutzerklärung, Secrets | M | **größtenteils erledigt** (2026-08-18): Unmanaged Fly Postgres angelegt und produktiv angebunden (`watchparty-league-db`, ADR-035-Nachtrag) — Strato-Hosting-DB vorher geprüft und verworfen (MySQL statt Postgres, private IP nicht erreichbar). Feed-Zugriff von ESPN blockiert (403 aus Fly.ios IP-Bereich) und durch einen täglichen GitHub-Actions-Relay ersetzt (ADR-037-Nachtrag) — GitHubs eigene Benachrichtigung bei fehlgeschlagenem Workflow ersetzt den ursprünglichen `AlertSender`-Mechanismus, der mit dem entfernten `ScheduleSyncJob` hinfällig wurde. Alle Secrets gesetzt und deployt: Datenbank, SMTP (IONOS, nicht Strato wie ursprünglich angenommen), `admin.email`, `schedule.season-year`, `schedule.relay-token` (beidseitig, auch als GitHub-Actions-Secret `SCHEDULE_RELAY_TOKEN`), `login.base-url` (korrigiert auf die echte Domain `watchparty.fourteen-it.de`, nicht die Fly-eigene). Datenschutzerklärung (`docs/datenschutzerklaerung.md`) und Impressum (`docs/impressum.md`, Angaben von fourteen-it.de) ins Frontend eingebunden und live (`/datenschutz`, `/impressum`) — Auftragsverarbeitungsverträge, Drittlandtransfer bei Fly.io und rechtliche Prüfung bleiben offen. **Offen:** Rückspielprobe der Datenbanksicherung (noch nicht durchgeführt) |
