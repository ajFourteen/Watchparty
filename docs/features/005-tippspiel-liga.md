...
| 8 | Betrieb | Sicherung der Datenbank, Überwachung des Feeds, Datenschutzerklärung, Secrets | M | **größtenteils erledigt** (2026-08-18): Feed-Alarm (`AlertSender`/`AlertMailSender`, drei fehlgeschlagene Läufe in Folge, `docs/betrieb-tippspiel.md`) umgesetzt und getestet. Unmanaged Fly Postgres angelegt und produktiv angebunden (`watchparty-league-db`, ADR-035-Nachtrag) — Strato-Hosting-DB vorher geprüft und verworfen (MySQL statt Postgres, private IP nicht erreichbar). Alle Secrets gesetzt und deployt: Datenbank, SMTP (IONOS, nicht Strato wie ursprünglich angenommen), `admin.email`, `alert.email`, `schedule.season-year`, `login.base-url`. Datenschutzerklärung (`docs/datenschutzerklaerung.md`) und Impressum (`docs/impressum.md`, Angaben von fourteen-it.de) ins Frontend eingebunden und live (`/datenschutz`, `/impressum`) — Auftragsverarbeitungsverträge, Drittlandtransfer bei Fly.io und rechtliche Prüfung bleiben offen. **Offen:** Rückspielprobe der Datenbanksicherung (noch nicht durchgeführt), echter Alert-Kanal für den Format-Bruch-Fall (13.7, log-basiert, kein Produktivcode) |
